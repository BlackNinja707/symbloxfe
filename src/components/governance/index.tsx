...
const Governance = () => {
	...
	const isVisibleHowDoesGovernanceWork = useScrollVisibility(
		"governance-how-does-governance-work",
		0,
	);
	...
	return (
		<div className="min-h-screen overflow-x-hidden h-full w-full text-white px-6 sm:px-24 font-Barlow">
			<div
				id="governance-main"
				className="flex flex-col md:flex-row justify-center items-center pt-[88px] gap-x-8 mb-36 mt-10"
			>
				<div>
					<div
						className={`transition-all duration-500 ${
							isVisibleMain ? "scale-100" : "scale-50"
						} text-white text-3xl sm:text-5xl font-bold max-w-[400px]`}
					>
				...
				<div
					className={`transition-all duration-500 ease-in delay-300 ${
						isVisibleGovernance
							? "opacity-100 translate-y-0"
							: "opacity-0 translate-y-20"
					} text-primaryText text-3xl sm:text-4xl font-bold`}
				>
			...
			<div id="governance-how-does-governance-work" className="mb-20">
				<div
					className={`transition-all duration-500 ease-in ${
						isVisibleHowDoesGovernanceWork
							? "opacity-100 translate-y-0"
							: "opacity-0 translate-y-20"
				...
				<div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
					{howDoesGovernanceWork.map((item, index) => (
						<div
							key={index}
							className={`transition-all duration-500 ease-in delay-[${
								(index + 1) * 200
							}ms] ${
								isVisibleHowDoesGovernanceWork
									? "opacity-100 translate-y-0"
									: "opacity-0 translate-y-20"
							} flex-col bg-primaryBoxColor p-10 rounded-xl sm:max-w-[400px] min-h-[350px]`}
						>
							<img
								src={item.img}
								alt={item.title}
